      ...
      function addListeners(){
        for (let i = 0; i < talen.length; i++){
          talen[i].addEventListener('click', function(evt){
            if (hold == null) {
            camera.innerHTML += '<a-box id="js--hold" class="js--talen js--interact" color="'+ this.getAttribute("color") + '" height='+ this.getAttribute("height") +' rotation="0 0 90" position="2 -2 -4" depth="0.1" width="0.8">'+ this.innerHTML + '</a-box>'
            hold = "box";
            console.log("true");
            }
          });
